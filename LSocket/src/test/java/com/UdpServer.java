...
public class UdpServer extends IoHandlerAdapter {
    ...
    public static final int MAX_RECEIVED = 5;
    ...
    public void messageReceived(IoSession session, Object message) throws Exception {
        ...
        System.out.println(DateTimeTool.getDateTime(new Date())+" messages"+message+"  "+session.isReadSuspended());
        synchronized (this){
            num++;
            if(num== 2){
                System.out.println(DateTimeTool.getDateTime(new Date())+"close=======" + nb );
                session.suspendRead();
            }else if(num== 3){
                System.out.println(DateTimeTool.getDateTime(new Date())+"回复" + nb);
              //  session.resumeRead();

            }
        }

        Object nbObject = session.getAttribute("nb");
        System.out.println(message+"====================>nb:"+(nbObject == null?"":nbObject));
    ...
    public void sessionCreated(IoSession session) throws Exception {
        //session.closeNow();
        System.out.println("Session created...num:");
    }
