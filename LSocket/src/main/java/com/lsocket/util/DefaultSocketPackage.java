...
public class DefaultSocketPackage {
    private final static int compresseMinLength = 150;
    ...
    public static void detransMsg(IoBuffer buffer) {
        try {
            ...
            int compreType = header>>PackageType.compresseType&3;
            System.out.println("compreType:"+compreType);
    ...
    public static IoBuffer transformErrorMsg(byte[] bytes,int compressType) {
        try {
           final int messageLength = bytes.length;
            IoBuffer buffer;
            if(messageLength>'\uffff'){
                byte packageType = (byte) (0|(compressType<<PackageType.compresseType)|0|1<<PackageType.isBiggerData|(1<<PackageType.classType));
                buffer = IoBuffer.allocate(messageLength + 3);
                buffer.put(packageType);
                buffer.putShort((short) messageLength);
            }else {
                byte packageType = (byte) (0|(compressType<<PackageType.compresseType)|0|0|(1<<PackageType.classType));
    ...
    public static IoBuffer transformByteArray(byte[] bytes,int compressType,byte module,byte cmd) {
        try {
            final int messageLength = bytes.length;

            IoBuffer buffer;
            if(messageLength>'\uffff'){
                byte packageType = (byte) (0|(compressType<<PackageType.compresseType)|1<<PackageType.cmdModuleContain|1<<PackageType.isBiggerData|0);
                buffer = IoBuffer.allocate(messageLength + 5);
                buffer.put(packageType);
                buffer.putShort((short) messageLength);
            }else {
                byte packageType = (byte) (0|(compressType<<PackageType.compresseType)|1<<PackageType.cmdModuleContain|0|0);
    ...
    final class PackageType{
        ...
        private static final int compresseType = 1+heart;
        private static final int cmdModuleContain = 2+compresseType;
