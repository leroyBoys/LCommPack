...
//        System.out.println(NumberUtils.toInt("-0000000002"));
    ...
    public final static boolean isBoolean(String str) {
        return match(str.toLowerCase(),"^(true)|(false)|(0)|(1)$");
    }
    ...
    public static boolean isContainsSpecialChar(String text) {
        if(StringTool.isEmpty(text)) return false;
        String[] chars={"[","`","~","!","@","#","$","%","^","&","*","(",")","+","=","|","{","}","'",
                ":",";","'",",","[","]",".","<",">","/","?","~","！","@","#","￥","%","…","&","*","（","）",
                "—","+","|","{","}","【","】","‘","；","：","”","“","’","。","，","、","？","]"};
        for(String ch : chars){
