...
public class Endecrypt {
    private final static Endecrypt endecrypt = new Endecrypt();
    private Endecrypt(){}
    ...
    public byte[] get3DESEncrypt(byte[] src, String spkey) {
        byte[] reData = null;
        try {
            //得到3-DES的密钥匙
            byte[] enKey = getEnKey(spkey);
    ...
    public byte[] get3DESDecrypt(byte[] bytes, String spkey) {
        try {
            Cipher cipher = Cipher.getInstance("DESede");
            byte[] key = getEnKey(spkey);
            DESedeKeySpec dks = new DESedeKeySpec(key);
